...
create table if not exists
    Encomenda(id int identity, idUser varchar(45),varchar(45) final);

create table if not exists
    Encomenda_Produtos(idEnc int ,idProd int ,quantidade int,PRIMARY KEY(idEnc,idProd));

create table if not exists
    StateUpdate(timestam int identity , typ int , userId varchar(45),idEnc int,inicial varchar(45),final varchar(45), idProdAdd int , qntProdAdd int );
